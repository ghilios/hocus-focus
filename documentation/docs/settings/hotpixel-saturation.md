...
!!! note "The panel's FWHM MAD can read higher afterwards"
    **FWHM MAD** is the spread of fitted widths over the whole frame, so it includes the variation of focus *across* the sensor. The median filter was flattening that variation along with everything else, which made the spread look tighter than the optics are. With the repair on, the frame's real focus gradient comes through — on one measured frame it went from 0.30 px to 0.46 px top to bottom — and the MAD rises with it, by about 10% on the same stars. Per-star precision moves the other way: the scatter left after the field shape is removed improves by about 12%. A higher MAD here means the panel is reporting the field you actually have, not a worse measurement.
...
!!! tip "When this helps"
    Turn it **on** if you care about FWHM, eccentricity or the aberration inspector's focus gradient, and re-run the optimization wizard afterwards so the gates are calibrated against it. Measured across the auto-focus bank it improves per-star FWHM precision on 16 of 19 runs, by about 22% at the median. Structure detection is unaffected either way, and autofocus accuracy is unchanged.
